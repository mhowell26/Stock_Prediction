...
MODEL_INFO = {
        ...
        "explainer": 'explainer_kernel_pca.shap', 
        "pipeline": 'finalized_kernel_pca_model.tar.gz', 
        "keys": ["IBM_CR_Cum","NVDA_CR_Cum"], 
        "inputs": [{"name": k, "type": "number", "min": 0.0, "max": 500.0, "default": 100.0, "step": 1.0} for k in ["IBM_CR_Cum","NVDA_CR_Cum"]] 
}
...
def call_model_api(input_df):
    ...
    try:
        ...
        pred_val = float(np.array(raw_pred).reshape(-1)[-1])
...
def display_explanation(input_df, session, aws_bucket):
    ...
    try:
        ...
        feature_names = [f'KPCA_Feature_{i}' for i in range(input_df_transformed.shape[1])]
